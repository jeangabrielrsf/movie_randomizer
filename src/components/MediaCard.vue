<template>
  <v-card class="media-card mx-auto" max-width="450" elevation="10" rounded="xl">
    ...
    <v-card-text class="pt-4 pb-0">
      ...
      <div class="text-center mb-2">
          <v-btn 
            v-if="alternatives && alternatives.length > 1" 
            variant="text" 
            size="x-small" 
      ...
      <div class="mb-2 text-center" @click="expandOverview = !expandOverview" style="cursor: pointer;">
        <div class="text-body-2" :class="{'text-truncate': !expandOverview}" style="max-height: 80px; overflow-y: auto;" v-if="expandOverview">
      ...
      <div v-if="providers && (providers.flatrate || providers.rent || providers.buy)" class="mb-4">
        ...
        <div class="d-flex justify-center flex-wrap gap-2">
           <v-tooltip location="top" v-for="prov in (providers.flatrate || []).slice(0, 3)" :key="prov.provider_id" :text="prov.provider_name">
    ...
    <v-card-actions class="justify-space-evenly pa-2">
      <v-btn icon="mdi-thumb-down" color="red-lighten-1" variant="tonal" size="small" @click="$emit('rate', 'negative')"></v-btn>
      <v-btn icon="mdi-heart" color="pink-accent-3" variant="flat" size="large" elevation="4" @click="$emit('rate', 'heart')"></v-btn>
      <v-btn icon="mdi-thumb-up" color="green-lighten-1" variant="tonal" size="small" @click="$emit('rate', 'positive')"></v-btn>
    </v-card-actions>
        ...
        <v-card-text style="max-height: 400px; overflow-y: auto;">
          <v-list lines="two">
             <v-list-item
                ...
                :prepend-avatar="getImage(item.poster_path)"
                @click="selectAlternative(item)"
             >
             </v-list-item>
          </v-list>
        </v-card-text>
